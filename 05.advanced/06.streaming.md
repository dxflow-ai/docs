...
The web console streams live, list-heavy views automatically: file-manager browsing/search/bulk-ops/archives, workflow live logs and status/metrics, and system status lists (shell sessions, health checks).
...
A response that arrives incomplete usually means the client doesn't handle chunked encoding — verify with `curl`, and fall back to `?stream=false`.
