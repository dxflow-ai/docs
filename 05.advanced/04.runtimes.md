...
Each runtime takes three values, set in the active profile (`~/.dxflow/<profile>_config.yaml`) or via environment variables:
...
Defaults per runtime (`*-args` default to `[]`):

| Runtime | Path | Registry | Env Prefix |
|----------|------|----------|------------|
...
This is what lets a runtime's `path` point at a wrapper (`orb`, `limactl`, `wsl`) that reaches the real binary inside a Linux VM.

## Running through a VM (macOS & Windows)

There is no native runtime on macOS or Windows: the engine runs natively, but containers run inside a Linux VM. Point each runtime `path` at a wrapper that forwards commands into the VM. Docker needs `sudo` (root-owned daemon); Podman, Singularity, and Apptainer are rootless.
...
Boot at `debug` level — the engine logs the resolved command for every runtime as it loads, so you can confirm a wrapper is applied before any container command runs:
...
```log
DBG Loaded docker runtime config path=limactl args=["shell","dxflow","sudo","docker"]
DBG Loaded podman runtime config path=limactl args=["shell","dxflow","podman"]
DBG Loaded singularity runtime config path=limactl args=["shell","dxflow","apptainer"]
DBG Loaded apptainer runtime config path=limactl args=["shell","dxflow","apptainer"]
```
...
| GPU flag | `--gpus all`, from any of `nvidia`, `cuda`, `amd`, `rocm`, `all` | `--nv` (`nvidia`/`cuda`) or `--rocm` (`amd`/`rocm`); `all` is not accepted |

For the full list of runtime endpoints (login, image, container/instance lifecycle), see the [Runtime API](/docs/api/runtime).
