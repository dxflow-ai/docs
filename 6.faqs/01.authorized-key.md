...
- **`-m PEM`** - Writes the private key in PEM (PKCS#1) format
- **`-C`** - Adds a comment (typically your email)

::callout{color="amber"}
**`-m PEM` is required.** dxflow reads PKCS#1/PKCS#8 PEM private keys, not OpenSSH's default `-----BEGIN OPENSSH PRIVATE KEY-----` format. Without `-m PEM`, the public key still registers, but the private key won't parse when you authenticate. If you already created a key without it, convert in place: `ssh-keygen -p -m PEM -f <private-key>`.
::
...
Use the **IDENTITY** shown by `dxflow key list`.
