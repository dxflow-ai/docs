...
dxflow uses RSA challenge-response: the client requests a challenge, the server returns a random nonce, the client signs it with the private key (RSA-PKCS1v15 + SHA256), and the server verifies the signature and issues a JWT token, which is stored in the profile config.
