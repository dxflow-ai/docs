...
This tree is the volume a workflow reaches by mounting `./volume`, and a directory in it — `data/` here — is the `./volume/data` a step names as its [host path](/docs/interface/workflows). So what you upload here is what a step reads, and what a step writes appears here.
...
| **New directory** | section / directory | Creates `new-directory` in rename mode — Enter keeps it, Esc or clicking away discards it |
