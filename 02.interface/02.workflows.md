...
- **Workflow node**: shows a colored **status dot**, the workflow name, and when it was created. Clicking it opens the workflow's **Diagram** and **Logs**.
...
Right-click a workflow node to **start**, **stop**, and **remove** it — removing asks for confirmation, then stops its containers and closes its tabs.

Step cards carry their own controls: **Start** on the first step while the workflow is `created`, `stopped`, or `exited`, **Stop** on the latest running step while the workflow is `started`, and **Shell** on a running step, which opens a terminal inside that step's container as a new entry under [Shells](/docs/interface/shells).
