...
Some launch-time values can be tuned per run **without editing the definition**, via a `workflow.ini` file in the workflow's folder (edit it under **Artifacts**). It is read **only at start** — changes take effect on the next start, not on a running workflow. Two categories are supported, each scoped to a step by its `name`:
...
`[env]` sets or replaces an environment variable on that step; `[resource]` replaces `cpu`, `memory`, or `gpu`. Everything else (image, command, volumes, ports) stays as defined. Overrides can also be passed when starting from the CLI (`--override`) or API.

Overrides never modify the workflow definition (`workflow.json`) — they are merged into the launch only. If an override targets a step `name` that does not exist, or gives an invalid `cpu`/`memory`/`gpu` value, the **start fails** with an `override: …` error.
