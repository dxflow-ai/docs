...
Each step accepts: `name`, `runtime` (default `docker`), `mode` (`parallel` or `sequential`, default `sequential`), `image`, `command`, `volumes` (`name`/`host`/`container`/`mode`), `ports` (`name`/`host`/`container`/`protocol`), `env`, and `resources` (`cpu`, `memory`, `gpu`). `volumes` and `ports` are objects that each require a `name` — a stable label used to target the mapping in an [override](#per-run-overrides).
