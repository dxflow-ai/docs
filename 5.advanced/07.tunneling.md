...
::card-group
  ...
  **Local Service Gateway**
  ...
  - Validates upstream is a private/loopback/unspecified address
  ::
::
...
| **Generation** | Held in a fixed 12-byte array, zero-padded if shorter |
...
::tabs
  ...
  # Remove a specific proxy allocation
  ...
  ::callout
  **Note**: There is no `proxy create` command — proxy allocations are created automatically when a bridge connects. The proxy CLI only supports `list`, `remove`, and `prune`.
  ::
  ::
...
Every proxy-bridge connection uses a lowercase identity of 8–12 characters:

| Property | Details |
|----------|---------|
| **Format** | 2 letters + 6–10 alphanumerics, lowercase (e.g., `ab1234567890`); stored in a 12-byte zero-padded buffer |
...
::tabs
  ...
  **Bridge Nodes**
  - **Default connections**: unlimited (`-1`) until a license sets a limit
  - **Default traffic**: unlimited (`-1`) until a license sets a limit
  - **License required**: Bridge functionality needs a license
...
::card-group
  ...
  **Local Addresses Only**
  - Upstream must be a private, loopback, or unspecified IPv4 address
  - Scheme must be `http` with an explicit port and empty path
  - Prevents exposing public services beyond the local network
  ::
...
::tabs
  ...
  **"Traffic limit exceeded"**
  - The license-defined traffic limit was reached on the bridge
...
# Reconnect specific bridge (identity in the body/query, not the path)
