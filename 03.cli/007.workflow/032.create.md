...
| `--fit` | `-f` | Cap every step's cpu and memory to the host capacity, alongside a start | `bool` | `false` |
...
`--fit` goes with `--start` and caps each step's `cpu` and `memory` to the engine host's core count and total memory, so a definition asking for more than the host has still starts. See [`workflow start`](/docs/cli/workflow/start).
