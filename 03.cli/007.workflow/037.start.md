...
`--link` assigns a link to every step declaring one under `link` in the definition, while the license carries any. Left out, the start runs the steps with their declared links idle. The success line carries what the start assigned, one `[url -> step]` detail per link.
